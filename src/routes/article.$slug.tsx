...
function ArticlePage() {
  ...
  if (missing) {
    return (
      <div className="mx-auto max-w-3xl py-16 text-center">
        ...
        <Link to="/" className="mt-6 inline-flex min-h-11 items-center rounded-full bg-crimson px-4 text-sm font-semibold text-paper">गृहपृष्ठ</Link>
      </div>
    );
  }
  ...
  const related = edition.filter((a) => a.slug !== article.slug && a.category === article.category).slice(0, 2);
        ...
        <div className="min-w-0">
          <div className="overflow-hidden rounded-[1.75rem] border border-line bg-white shadow-sm">
            <header className="px-5 pt-6 text-center sm:px-8">
              <p className="text-[11px] font-bold tracking-[0.16em] text-[#14934e]">{categoryLabel(cats, article.category)}</p>
              ...
              <div className="mt-4 flex flex-wrap items-center justify-center gap-2 pb-2">
                <TextResizer />
                <button type="button" onClick={() => toggleSaved(article.slug)} className="inline-flex min-h-11 items-center gap-2 rounded-full border border-line bg-paper px-3 text-sm font-medium hover:border-crimson">
            ...
            {article.excerpt ? (
              <p className="mx-5 mt-4 rounded-2xl bg-[#f6f6f6] px-5 py-4 text-center text-lg leading-relaxed sm:mx-8">{article.excerpt}</p>
            ) : null}
            {article.imageUrl ? (
            ...
            <div className="px-5 py-8 sm:px-8">
              <div className="article-body space-y-6 font-display leading-[1.9] text-ink" style={{ fontSize: `${1.15 * textScale}rem` }}>
                {paragraphs.map((p, i) => (
              ...
              {article.tags.length ? (
                <div className="mt-8 flex flex-wrap justify-center gap-2">
                  {article.tags.map((tag) => (
                    <Link key={tag} to="/search" search={{ q: tag }} className="rounded-full bg-[#e7f4eb] px-3 py-1 text-xs font-semibold text-[#0b6b38] hover:bg-[#14934e] hover:text-white">
                      #{tag}
                    </Link>
                  ))}
                </div>
          ...
          <div className="mt-8">
            ...
            {related.length ? (
              <section className="mt-10">
                <h2 className="mb-4 text-center font-display text-2xl">सम्बन्धित {categoryLabel(cats, article.category)}</h2>
